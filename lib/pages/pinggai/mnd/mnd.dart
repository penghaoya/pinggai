...
class Mnd extends StatefulWidget {
  const Mnd({super.key});

  @override
  MndState createState() => MndState();
}

class MndState extends State<Mnd> with SingleTickerProviderStateMixin {
  // 状态栏
  int userTotal = 0;
  int keyTotal = 0;
  int allTaskTotal = 0;
  ...
  final axios = Axios(
    baseUrl: 'https://2024utc.mirinda.com.cn',
    defaultHeaders: {
      'Origin': 'https://2024utc.mirinda.com.cn',
      'Referer': 'https://2024utc.mirinda.com.cn/dist/index.html?v=shukkaxin',
    },
  );
    ...
    processTasks(
      ...
      doSomething: (index, assignment) async {
        ...
        try {
          ...
          List<int> bytes = utf8.encode(match?.group(1) as String);
          // 进行Base64编码
          String encodedString = base64Encode(bytes);
          // POST 请求示例
          final postResponse = await axios.post(
            '/pincode/exchange',
            data: {"code": encodedString, "fxtoken": assignment['user']},
            headers: {
              'Cookie': assignment['user'],
            },
          );
          LogManager().info(postResponse);
          String code = postResponse['status'];
          if (code == '101') {
            Tips.info("检查用户是否有效");
          ...
          setState(() {
            _data.add({
              ...
              'result': code == '200'
                  ? '${postResponse['object']['bgoods']['goodsName']}'
                  : '${postResponse['msg']}',
            });
