...
class BaiShi extends StatefulWidget {
  const BaiShi({super.key});

  @override
  BaiShiState createState() => BaiShiState();
}

class BaiShiState extends State<BaiShi> with SingleTickerProviderStateMixin {
  // 状态栏
  int userTotal = 0;
  int keyTotal = 0;
  int allTaskTotal = 0;
  ...
  final TextEditingController _sleepMaxController = TextEditingController();

  // 请求实例
  final axios = Axios(
    baseUrl: 'https://2024blueutc.pepsi.com.cn',
    defaultHeaders: {
      'Content-Type': 'application/json',
      'Referer':
          'https://servicewechat.com/wx72f057eca900725f/86/page-frame.html',
    },
  );
    ...
    processTasks(
      ...
      doSomething: (index, assignment) async {
        ...
        try {
          // 扫码
          late String result;
          final postResponse = await axios.post(
            '/pincode/scanCode.do',
            data: {"pinCode": assignment['task'], "token": assignment['user']},
          );
          if (postResponse['status'] == '104') {
            Tips.info("检查用户是否有效");
            LogManager().info('${postResponse['msg']}--${assignment['user']}');
            return;
          }
          if (postResponse['status'] == '507') {
            Tips.info("今日扫码次数已达上限，明日再来哦");

            return;
          }
          print('postResponse:-------$postResponse');
          if (postResponse['status'] == '505') {
            result = '该二维码已被使用';
            print("wwwwwwwww二维码-------------");
          } else {
            // 扫码
            int codeId = postResponse['object']['blotteryMallRecord']['id'];
            print(codeId);
            final codeLimit = await axios
                .get('/pincode/codeLimit.do?token=${assignment['user']}');
            print("codelimit:-------$codeLimit");
            final exchange = await axios.post(
              '/pincode/exchange.do',
              data: {"lotteryId": codeId, "token": assignment['user']},
            );
            print("exchange---------: $exchange");
            if (exchange['status'] == '200') {
              result = exchange['object']['bgoods']['goodsName'];
              LogManager().info("抽奖结果：$result");
            }
          }
          setState(() {
            _data.add({
